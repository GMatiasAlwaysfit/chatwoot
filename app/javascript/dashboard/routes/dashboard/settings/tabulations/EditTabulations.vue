<template>
  <div>
    <div class="h-auto overflow-auto flex flex-col">
      <woot-modal-header :header-title="'Editar tabulação'" />
      <div class="h-auto overflow-auto flex flex-col">
        <form class="flex flex-col w-full" @submit.prevent="editTabulation()">
          <div class="w-full">
...
export default {
  props: {
    ...
    eddescription: { type: String, default: '' },
    edendPhrase: { type: String, default: '' },
    edtabType: { type: String, default: '' },
  ...
  data() {
    return {
      ...
      endPhrase: this.edendPhrase,
      tabType: this.edtabType,
      addTabulations: {
        showLoading: false,
        message: '',
      },
  ...
  methods: {
    ...
    async editTabulation() {
      this.addTabulations.showLoading = true;
      try {
        await this.$store.dispatch('tabulation/updateTabulation', {
          id: this.id,
          name: this.name,
        ...
        bus.$emit('newToastMessage', 'Tabulação atualizada com sucesso');
        this.resetForm();
        ...
        this.addTabulations.showLoading = false;
        const errorMessage =
          error.message || 'Ocorreu um erro ao atualizar a tabulação';
